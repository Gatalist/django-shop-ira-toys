	...
	$.ajax({
		...
		success: function(data) {
			...
			data_products.forEach(element => {
				addInBasket(element, data.valuta);
				all_sum += element.sum;
			});
			// сумма заказа в корзине header
			let tag_all_sum = document.querySelector("#all_sum");
			tag_all_sum.innerText = all_sum.toFixed(2) + data.valuta;
			// таблица заказа
			let all_sum_table = document.querySelector(".total-amount-table");
			if (all_sum_table) {
				all_sum_table.innerText = all_sum.toFixed(2) + data.valuta;
				if (all_sum == 0) {
...
let addInBasket = (element, valuta) => {
	...
	price.innerText = (element.sum).toFixed(2) + valuta;
